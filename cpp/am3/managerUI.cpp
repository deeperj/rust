...
void Amtui::displayReg() {
	drawBorder();
	Point2D tmp[] = { t[ebx],t[pbx],t[vbx],t[mbx],t[obx],t[cbx]};
	tab_cycler t2[] = { ebx,pbx,vbx,mbx,obx,cbx };
	configure(tmp, t2, (sizeof(t2) / sizeof(t2[0])));
...
void Amtui::displayForgotPass() {
	...
	tab_cycler t2[] = { ebx,pbx,vbx,mbx,obx,cbx };
	configure(tmp, t2, (sizeof(t2) / sizeof(t2[0])));
	session.setStatus(ForgotPasswordSession);
	move(1, 1);
	...
	while (char c = getch()) {//tab=9
		...
		case -53:
			if (c == ' ') {
				...
				if (ctab == id(vbx)) {
					printw("*");
					session.user.PasswordVerify.append(1, c);
				}
				break;
			}
			if (ctab == id(obx)) {
		...
		default:
			...
			if (ctab == id(mbx)) {
				if (int(c) != 127)printw("%c",c);
				session.user.Maiden.append(1, c);
			}
			break;
		}
	}
