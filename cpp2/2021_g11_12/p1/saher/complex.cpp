...
Complex Complex::Multiplication(Complex Q) {
    Complex output = Complex(Q.Real() * mReal - Q.Imag() * mImag, Q.Real() * mImag + Q.Imag() * mReal);
    return output;
}
    
Complex Complex::Addition(Complex Q) {
    Complex output = Complex(Q.Real() + mReal , Q.Imag()+mImag);
    return output;
        
}

Complex Complex::Subtraction(Complex Q) {
    Complex output = Complex(Q.Real() - mReal , Q.Imag()-mImag);
        return output;
}

Complex Complex::Division(Complex Q) {
    Complex output = Complex( (Q.Real() * mReal + Q.Imag() * mImag) / (mReal * mReal + mImag * mImag) , ( Q.Imag() * mReal - Q.Real() * mImag)/ (mImag * mReal + mImag * mImag));
    return output;
}
