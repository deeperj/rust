...
class ComputeEnumerable {
    ...
    enumerate() {
        return functions.pipe([this.f()], fromArray(), enumerate);
    }
}
...
class LiftedEnumerableLike {
    ...
    enumerate() {
        ...
        return functions.pipe(src, ...this.operators);
    }
}
/**
 ...
 * @param mapper
 */
const concatMap = (mapper) => functions.compose(map(mapper), concatAll());
...
function endWith(...values) {
    return functions.pipe(values, fromArray(), concatWith);
}
...
const _fromValue = (value) => functions.pipe([value], fromArray());
/**
...
class TakeLastEnumerator {
    ...
    move() {
        ...
        if (option.isNone(this.enumerator)) {
            ...
            this.enumerator = functions.pipe(last, fromArray(), enumerate);
        }
        this.enumerator.move();
        return this.hasCurrent;
    }
...
const enumeratorToRunnable = (f) => {
    ...
    return runnable.createRunnable(run);
};
...
class ZipEnumerable {
    ...
    enumerate() {
        return functions.pipe(this.enumerables, readonlyArray.map(enumerate), zipEnumerators);
    }
}
/**
