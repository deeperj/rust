...
class SetMultimap {
    ...
    enumerate() {
        return functions.pipe(functions.defer(this, iterateSetMultimapKeyValuePairs), enumerable.fromIterator(), enumerable.enumerate);
    }
    get(key) {
        var _a;
...
const tryDispatch = (resourceManager, key) => {
    ...
    disposable.addTeardown(observer, () => {
        ...
        const timeoutSubscription = functions.pipe(observable.fromValue({ delay: maxIdleTime })(option.none), observable.subscribe(scheduler, _ => {
...
function onDispose(error) {
    const forEachDispose = runnable.forEach(disposable.dispose(error));
