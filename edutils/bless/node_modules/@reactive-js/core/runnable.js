...
const _compute = (f) => createRunnable(sink => functions.pipe(f(), fromValue()).run(sink));
...
function repeat(predicate) {
    const shouldRepeat = option.isNone(predicate)
...
const takeLast = (options = {}) => {
    ...
    return runnable => (count > 0 ? functions.pipe(runnable, lift(operator)) : empty());
};
...
exports.AbstractDelegatingSink = AbstractDelegatingSink;
